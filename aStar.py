class missionariesCannibalsGraph:
    ...
    def h(self, n):
        h_n_cost = {'A': 30 / 2, 'B': 35 / 2, 'C': 24 / 2, 'D': 24 / 2, 'E': 20 / 2, 'F': 15 / 2, 'G': 15 / 2,
                    'H': 12 / 2, 'I': 12 / 2,
                    'J': 8 / 2, 'K': 8 / 2, 'L': 8 / 2, 'M': 6 / 2, 'N': 6 / 2, 'O': 3 / 2, 'P': 2 / 2, 'Q': 3 / 2,
                    'R': 2 / 2, 'S': 6 / 2, 'T': 0 / 2
                    }
        return h_n_cost[n]
    ...
    def aStar(self, source, target):
        ...
        closedList = set([])
        # the distance dictionary provides
        distance = {}
        distance[source] = 0
        # Here we initialized adjacent
        adjMapping = {}
        adjMapping[source] = source
        while len(openedList) > 0:
            n = None
            # finding minimum f(n) node
            for v in openedList:
                if n == None or distance[v] + self.h(v) < distance[n] + self.h(v):
                    n = v;
            if n == None:
                print('No path found')
                return None
            # if current state is target
            # then we start again from the source
            if n == target:
                buildPath = []
